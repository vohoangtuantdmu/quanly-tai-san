...
import {
  CONTACT_TYPE, CONTRACT_DIRECTION, PAYMENT_CYCLE, TAX_RESPONSIBILITY, enumOptions,
  type ContactTypeCode, type ContractDirectionCode, type PaymentCycleCode, type TaxResponsibilityCode,
...
function NewContract() {
  ...
  const create = useMutation({
    ...
    onSuccess: (created) => {
      toast.success("Đã tạo hợp đồng. Hệ thống đã tự tạo 2 nhắc lịch.", { icon: <Sparkles className="h-4 w-4" /> });
    ...
    onError: (err) => {
      if (err instanceof ApiError && err.status === 409) {
        toast.error(getErrorMessage(err), { description: "Hãy xem lại danh sách hợp đồng của tài sản này." });
      } else {
  ...
  const canNext =
    ...
    (step === 2 && !!startDate && !!endDate && (rentAmount ?? 0) > 0 && paymentDueDay >= 1 && paymentDueDay <= 31 && new Date(endDate) > new Date(startDate)) ||
    step === 3;
  ...
  return (
    <div className="p-6 max-w-3xl space-y-5">
      <Button variant="ghost" size="sm" asChild><Link to="/hop-dong"><ArrowLeft className="h-4 w-4 mr-1" />Quay lại</Link></Button>
      <div>
          ...
          {step === 0 && (
            <>
              <div className="space-y-1.5">
                <Label>Tài sản *</Label>
                <Select value={assetId} onValueChange={(v) => { setAssetId(v); setScope("whole"); setAssetUnitId(""); }}>
                  <SelectTrigger><SelectValue placeholder="Chọn tài sản" /></SelectTrigger>
                  <SelectContent>
                    {(assetsQ.data?.items ?? []).map((a) => (
                      <SelectItem key={a.id} value={a.id}>{a.name} — {a.district}, {a.city}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              {assetId && hasUnits && (
                <div className="space-y-2">
                  ...
                  <RadioGroup value={scope} onValueChange={(v) => setScope(v as "whole" | "unit")}>
                    <div className="flex items-center gap-2"><RadioGroupItem value="whole" id="whole" /><Label htmlFor="whole" className="font-normal">Cho thuê nguyên căn</Label></div>
                    <div className="flex items-center gap-2"><RadioGroupItem value="unit" id="unit" /><Label htmlFor="unit" className="font-normal">Cho thuê theo tầng/phòng</Label></div>
                  </RadioGroup>
                  {scope === "unit" && (
                    <div className="pl-6 pt-1">
                      <Select value={assetUnitId} onValueChange={setAssetUnitId}>
                        ...
                        <SelectContent>
                          {availableUnits.length === 0 && <div className="p-2 text-xs text-muted-foreground">Không có phòng nào ở trạng thái Trống.</div>}
                          {availableUnits.map((u) => (
                            <SelectItem key={u.id} value={u.id}>{u.name}{u.floorNumber != null ? ` — tầng ${u.floorNumber}` : ""}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    </div>
              ...
              {assetId && !hasUnits && (
                <p className="text-xs text-muted-foreground">Tài sản không chia tầng/phòng — hợp đồng sẽ áp dụng cho toàn bộ tài sản.</p>
              )}
            </>
          )}
          ...
          {step === 1 && (
            <>
              <div className="space-y-2">
                ...
                <RadioGroup value={String(effectiveDirection)} onValueChange={(v) => { setDirection(Number(v) as ContractDirectionCode); setCounterpartyId(""); }}>
                  <div className="flex items-center gap-2"><RadioGroupItem value="1" id="d1" /><Label htmlFor="d1" className="font-normal">Tôi cho thuê</Label></div>
                  <div className="flex items-center gap-2">
                    <RadioGroupItem value="2" id="d2" disabled={isOwned} />
                    <Label htmlFor="d2" className={`font-normal ${isOwned ? "text-muted-foreground" : ""}`}>Tôi đi thuê</Label>
                    {isOwned && <span className="text-xs text-muted-foreground">(chỉ áp dụng cho tài sản đi thuê)</span>}
                  </div>
                </RadioGroup>
              </div>
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <Label>Đối tác * <span className="text-muted-foreground font-normal">— gợi ý: {CONTACT_TYPE[suggestedType]}</span></Label>
                  <Button size="sm" variant="ghost" onClick={() => setAddContactOpen(true)}><Plus className="h-3.5 w-3.5 mr-1" />Thêm đối tác</Button>
                </div>
                <Select value={counterpartyId} onValueChange={setCounterpartyId}>
                  <SelectTrigger><SelectValue placeholder="Chọn đối tác" /></SelectTrigger>
                  <SelectContent>
                    {contactOptions.preferred.map((c) => (
                      <SelectItem key={c.id} value={c.id}>{c.fullName} — {CONTACT_TYPE[c.type]}{c.phone ? ` · ${c.phone}` : ""}</SelectItem>
                    ))}
                    {contactOptions.others.length > 0 && contactOptions.preferred.length > 0 && (
                      <div className="px-2 py-1 text-xs text-muted-foreground border-t mt-1 pt-2">Khác:</div>
                    )}
                    {contactOptions.others.map((c) => (
                      <SelectItem key={c.id} value={c.id}>{c.fullName} — {CONTACT_TYPE[c.type]}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
          ...
          {step === 2 && (
            <>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5"><Label>Ngày bắt đầu *</Label><Input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} /></div>
                <div className="space-y-1.5"><Label>Ngày kết thúc *</Label><Input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} /></div>
              </div>
              <div className="space-y-1.5"><Label>Tiền thuê *</Label><CurrencyInput value={rentAmount} onChange={setRentAmount} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label>Chu kỳ thanh toán</Label>
                  <Select value={String(paymentCycle)} onValueChange={(v) => setPaymentCycle(Number(v) as PaymentCycleCode)}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>{enumOptions(PAYMENT_CYCLE).map((o) => <SelectItem key={o.value} value={String(o.value)}>{o.label}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div className="space-y-1.5"><Label>Ngày trả trong kỳ (1-31)</Label><Input type="number" min={1} max={31} value={paymentDueDay} onChange={(e) => setPaymentDueDay(Number(e.target.value))} /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5"><Label>Tiền cọc</Label><CurrencyInput value={depositAmount} onChange={setDepositAmount} /></div>
                <div className="space-y-1.5"><Label>Ngày tăng giá tiếp theo</Label><Input type="date" value={nextIncrease} onChange={(e) => setNextIncrease(e.target.value)} /></div>
              </div>
              <div className="space-y-1.5">
                <Label>Ai chịu thuế</Label>
                <RadioGroup value={String(taxResp)} onValueChange={(v) => setTaxResp(Number(v) as TaxResponsibilityCode)} className="flex gap-4">
                  <div className="flex items-center gap-2"><RadioGroupItem value="1" id="t1" /><Label htmlFor="t1" className="font-normal">Chủ nhà</Label></div>
                  <div className="flex items-center gap-2"><RadioGroupItem value="2" id="t2" /><Label htmlFor="t2" className="font-normal">Người thuê</Label></div>
                </RadioGroup>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Label htmlFor="act" className="font-normal">Kích hoạt ngay sau khi tạo</Label>
              </div>
            </>
          )}
          ...
          {step === 3 && (
            <div className="space-y-4">
              <div className="space-y-1 text-sm">
                ...
                <Row label="Phạm vi" value={effectiveScope === "unit" ? (availableUnits.find((u) => u.id === assetUnitId)?.name ?? "—") : "Nguyên căn"} />
                <Row label="Chiều" value={CONTRACT_DIRECTION[effectiveDirection]} />
                <Row label="Đối tác" value={contactsQ.data?.items.find((c) => c.id === counterpartyId)?.fullName ?? "—"} />
              ...
              <div className="rounded-md border border-info/40 bg-info/10 p-3 text-sm flex gap-2 items-start">
                ...
                <div>
                  ...
                  <ul className="mt-1 text-muted-foreground list-disc list-inside space-y-0.5">
                    ...
                    <li>Tạo nhắc lịch thu/đóng tiền định kỳ và nhắc hết hạn hợp đồng (trước 30 ngày)</li>
                  </ul>
                </div>
              </div>
            </div>
      ...
      <div className="flex justify-between">
        <Button variant="outline" onClick={() => (step === 0 ? navigate({ to: "/hop-dong" }) : setStep(step - 1))} disabled={create.isPending}>
        ...
        {step < STEPS.length - 1 ? (
          ...
          <Button onClick={submit} disabled={create.isPending}>{create.isPending ? "Đang tạo..." : "Tạo hợp đồng"}</Button>
        )}
      </div>
...
function Row({ label, value }: { label: string; value: string }) {
  return <div className="flex justify-between gap-3 py-1.5 border-b last:border-0"><span className="text-muted-foreground">{label}</span><span className="font-medium text-right">{value}</span></div>;
}

function QuickAddContactDialog({
  open, onOpenChange, defaultType, onCreated,
}: {
  open: boolean;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle>Thêm đối tác nhanh</DialogTitle></DialogHeader>
        <div className="space-y-3">
          ...
          <div className="space-y-1.5"><Label>Họ tên *</Label><Input value={fullName} onChange={(e) => setFullName(e.target.value)} /></div>
          <div className="grid grid-cols-2 gap-3">
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={mut.isPending}>Huỷ</Button>
          <Button
            onClick={() => {
              if (!fullName.trim()) { toast.error("Vui lòng nhập họ tên"); return; }
              mut.mutate({ type, fullName: fullName.trim(), phone: phone.trim() || null, email: email.trim() || null });
            }}
            disabled={mut.isPending}
          >{mut.isPending ? "Đang lưu..." : "Lưu"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
