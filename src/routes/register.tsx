...
function RegisterPage() {
  ...
  const [form, setForm] = useState({ email: "", name: "", password: "", confirm: "", phoneNumber: "" });
    ...
    <div className="flex min-h-screen items-center justify-center bg-muted/30 px-4 py-8">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">Đăng ký để bắt đầu quản lý tài sản của bạn.</p>
        </CardHeader>
        <CardContent>
          <form onSubmit={onSubmit} className="space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="email">Email</Label>
              <Input id="email" type="email" value={form.email} onChange={update("email")} disabled={busy} />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="phone">Số điện thoại <span className="text-muted-foreground">(tuỳ chọn)</span></Label>
              <Input id="phone" value={form.phoneNumber} onChange={update("phoneNumber")} disabled={busy} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="password">Mật khẩu</Label>
              <Input id="password" type="password" value={form.password} onChange={update("password")} disabled={busy} />
              {form.password && (
                <div className="space-y-1">
                  <div className="flex h-1.5 gap-1">
                    {[0, 1, 2, 3].map((i) => (
                      <div key={i} className={`flex-1 rounded ${i < strength.score ? strength.tone : "bg-muted"}`} />
                    ))}
                  </div>
            ...
            <div className="space-y-1.5">
              ...
              <Input id="confirm" type="password" value={form.confirm} onChange={update("confirm")} disabled={busy} />
            </div>
            {error && <div className="rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-sm text-destructive">{error}</div>}
            <div className="rounded-md border border-warning/40 bg-warning/10 px-3 py-2 text-xs text-warning-foreground">
              Ghi chú (chế độ dev): email xác thực được in ra console của backend. Copy link trong console và mở trong trình duyệt để xác thực.
            </div>
