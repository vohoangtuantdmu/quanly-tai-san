 ...
 */
export function LinkPropertyCard({
  assetId,
  linkedPropertyId,
}: {
  assetId: string;
  ...
  const linkMut = useMutation({
    mutationFn: (pid: number | string) => assetsApi.linkProperty(assetId, pid),
    onSuccess: () => {
  ...
  return (
    <Card>
      ...
      <Dialog
        open={openLink}
        onOpenChange={(v) => {
          if (!linkMut.isPending) setOpenLink(v);
        }}
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Nhập mã tin đăng (Property ID) để liên kết. Sẽ thay bằng danh sách chọn khi API sẵn
              sàng.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => propertyId && linkMut.mutate(propertyId)}
              disabled={!propertyId || linkMut.isPending}
            >
      ...
      <AlertDialog
        open={openUnlink}
        onOpenChange={(v) => {
          if (!unlinkMut.isPending) setOpenUnlink(v);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tài sản sẽ không còn liên kết với tin đăng #{linkedPropertyId}.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
